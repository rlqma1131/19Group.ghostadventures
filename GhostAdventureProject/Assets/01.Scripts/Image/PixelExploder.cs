...
public class PixelExploder : MonoBehaviour
{
    ...
    readonly List<GameObject> pixelPieces = new();
    Transform playerTransform;
    
    void Awake() => DOTween.SetTweensCapacity(5000, 200);
    
    public void Explode() {
        // 셰이더가 할당되었는지 확인
        if (!pixelParticleShader) {
        ...
        for (int x = 0; x < spriteRect.width; x += pixelStep) {
            for (int y = 0; y < spriteRect.height; y += pixelStep) {
                ...
                var pixelObj = GameObject.CreatePrimitive(PrimitiveType.Quad);
                ...
                // 할당된 셰이더로 새 머티리얼 생성
                var mat = new Material(pixelParticleShader);

                MeshRenderer renderer = pixelObj.GetComponent<MeshRenderer>();
                ...
                pixelObj.transform.DOMove(explodeTarget, explodeDuration)
                    ...
                    .OnComplete(() =>
                    {
                        if (playerTransform != null) {
                            Sequence absorbSeq = DOTween.Sequence();
                            absorbSeq.AppendInterval(delayBeforeAbsorb);
                            absorbSeq.Append(pixelObj.transform.DOMove(playerTransform.position, absorbDuration)
                                .SetEase(Ease.InCubic));
                            absorbSeq.Append(mat.DOColor(new Color(color.r, color.g, color.b, 0), "_BaseColor", 0.3f));
                            absorbSeq.Join(mat.DOColor(new Color(emissionColor.r, emissionColor.g, emissionColor.b, 0),
                                "_EmissionColor", 0.3f));
                            ...
                            fadeSeq.Join(mat.DOColor(new Color(emissionColor.r, emissionColor.g, emissionColor.b, 0),
                                "_EmissionColor", 0.3f));
                            fadeSeq.OnComplete(() =>
                            {
                                fadeSeq.Kill();
    ...
    public void RadialFallExplosion() {
        ...
        for (int i = 0; i < totalFragments; i++) {
            ...
            // Rigidbody (3D) 추가
            Rigidbody rb = pixelObj.AddComponent<Rigidbody>();
            ...
            fadeSeq.Join(mat.DOColor(new Color(emissionColor.r, emissionColor.g, emissionColor.b, 0), "_EmissionColor",
                3f));
            fadeSeq.OnComplete(() =>
            {
                fadeSeq.Kill();
    ...
    public void SnapToDust() {
        ...
        for (int x = 0; x < spriteRect.width; x += pixelStep) {
            for (int y = 0; y < spriteRect.height; y += pixelStep) {
                ...
                explodeSeq.Join(mat.DOColor(new Color(emissionColor.r, emissionColor.g, emissionColor.b, 0),
                    "_EmissionColor", duration));
