using TMPro;
using UnityEngine;

// 경비실 문에 붙이는 스크립트입니다.
// 빙의 상태에서 문에 닿았을 때 E키를 누르면 빙의해제 되지 않고 다른문으로 이동할 수 있게 됩니다.
public class Ch2_SecurityOfficeDoor : BaseDoor
{
    [SerializeField] private CH2_SecurityGuard guard;
    [SerializeField] private GameObject e_key;

    private bool moveAble = false;

    protected override void Start(){
        base.Start();
        isLocked = true;
    }
    protected override void Update()
    {
        base.Update();
        if(guard.doorLock.doorOpen && isLocked) isLocked = false;

    }

    protected override void TryInteract(){
        if(moveAble && guard.doorLock.doorOpen && !guard.IsPossessed)
        {
            TeleportPlayer();
        }
    }
    

    private void OnTriggerStay2D(Collider2D other) 
    {
        if (other.CompareTag("Person") || other.CompareTag("Player"))
        {
            guard.doorPass = true;
            moveAble = true;
            if(guard.doorLock.doorOpen)
            {
                e_key.SetActive(true);
            }
        }
    }

    protected override void OnTriggerExit2D(Collider2D collision)
    {
        if (collision.CompareTag("Person") || collision.CompareTag("Player"))
        {
            guard.doorPass = false;
            moveAble = false;
            if(guard.doorLock.doorOpen)
            {
                e_key.SetActive(false);
            }
        }
    }
}